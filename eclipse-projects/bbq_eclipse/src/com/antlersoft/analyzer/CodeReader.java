...
{
    public CodeReader( AnalyzeClass.CodeAttribute q)
    {
	ca=q;
	currentPos=0;
	la=null;
	...
	_methodCallback=null;
	_fieldCallback=null;
    }

    public OpCodeCallback setOpCodeCallback( OpCodeCallback newOpCodeCallback)
    {
	OpCodeCallback ret=_opCodeCallback;
	_opCodeCallback=newOpCodeCallback;
	return ret;
    }

    public ReferenceCallback setMethodInvokeCallback( ReferenceCallback newMethodCallback)
    {
	ReferenceCallback ret=_methodCallback;
	_methodCallback=newMethodCallback;
	return ret;
    }

    public ReferenceCallback setFieldReferenceCallback( ReferenceCallback newRefCallback)
    {
	ReferenceCallback ret=_fieldCallback;
	_fieldCallback=newRefCallback;
	return ret;
    }
	...
	if ( la!=null)
	{
	    for ( int i=0; i<la.lineNumberEntryCount; i++)
	    {
		if ( la.lineNumberEntries[i].start_pc<=position)
		{
		    lineNumber=la.lineNumberEntries[i].line_number;
		}
	    }
	}
	return lineNumber;
    ...
    int currentPos;
    private OpCodeCallback _opCodeCallback;
    private ReferenceCallback _methodCallback;
    private ReferenceCallback _fieldCallback;
	...
	try
	{
	    ...
	    new FieldReference( 180, 3, "getfield");
	    new FieldReference( 178, 3, "getstatic");
	    ...
	    new FieldReference( 181, 3, "putfield");
	    new FieldReference( 179, 3, "putstatic");
    ...
    {
	BadInstructionException( int opcode)
	{
	    super( "Unrecognized opcode: "+Integer.toString( opcode));
	}
    }
	...
	void read( CodeReader cr)
	{
	    if ( cr._methodCallback!=null)
	    {
		int operand=(mU( cr.ca.code[cr.currentPos+1])<<8)|mU( cr.ca.code[cr.currentPos+2]);
		cr._methodCallback.processReference( this, cr.currentPos, operand,
		    cr.getLineNumber( cr.currentPos));
	    }
	    super.read( cr);
	}
    }
    ...
    static class FieldReference extends OpCode
    {
	FieldReference( int v, int l, String m)
	{
	    super( v, l, m);
	}
	...
	void read( CodeReader cr)
	{
	    if ( cr._fieldCallback!=null)
	    {
		int operand=(mU( cr.ca.code[cr.currentPos+1])<<8)|mU( cr.ca.code[cr.currentPos+2]);
		cr._fieldCallback.processReference( this, cr.currentPos, operand,
		    cr.getLineNumber( cr.currentPos));
	    }
	    super.read( cr);
	}
    ...
    public static interface OpCodeCallback
    {
	public void processOpCode( OpCode read, int position);
    }

    public static interface ReferenceCallback
    {
	public void processReference( OpCode read, int position,
